...
$files=@("$root\dist\portable\BlackVoice.exe","$root\dist\installer\BlackVoice-Setup-x64.exe");foreach($f in $files){if(!(Test-Path -LiteralPath $f)){throw "Pacote ausente: $f"}}
