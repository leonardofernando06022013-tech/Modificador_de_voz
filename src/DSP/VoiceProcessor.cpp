...
void VoiceProcessor::prepare(double sr, int block, int channels) {
    ...
    pitch.prepare(sr,block); compressor.prepare(spec); limiter.prepare(spec); chorus.prepare(spec); delay.prepare(spec); flangerDelay.prepare(spec); reverb.prepare(spec);
    dry.setSize(channels,block); wet.setSize(channels,block);
    inputGain.reset(sr,0.02); outputGain.reset(sr,0.02); mix.reset(sr,0.02);
    formantAmount.reset(sr,0.04); reset();
}
void VoiceProcessor::reset(){ pitch.reset(); compressor.reset(); limiter.reset(); chorus.reset(); delay.reset(); flangerDelay.reset(); reverb.reset(); gateEnvelope=0; ringPhase=0; flangerPhase=0; hpX.fill(0); hpY.fill(0); lpY.fill(0); toneLow.fill(0); toneHigh.fill(0); formantLow.fill(0); dcX.fill(0); dcY.fill(0); }
void VoiceProcessor::process(juce::AudioBuffer<float>& b) noexcept {
    ...
    inputGain.setTargetValue(juce::Decibels::decibelsToGain(params.inputGainDb.load())); outputGain.setTargetValue(juce::Decibels::decibelsToGain(params.outputGainDb.load())); mix.setTargetValue(juce::jlimit(0.0f,1.0f,params.mix.load())); formantAmount.setTargetValue(juce::jlimit(-1.0f,1.0f,params.formant.load()));
    ...
    if(params.cleanupEnabled.load()){
        ...
        const float nr=juce::jlimit(0.0f,1.0f,params.noiseReduction.load()); for(int i=0;i<n;++i)for(int c=0;c<ch;++c){const float x=b.getSample(c,i),floor=0.006f*nr,level=std::abs(x);const float expansion=floor>0?juce::jlimit(1.0f-nr*.85f,1.0f,level/floor):1.0f;b.setSample(c,i,x*expansion);}
    }
    {
        const float lowAlpha=1.0f-std::exp(-juce::MathConstants<float>::twoPi*250.0f/static_cast<float>(sampleRate));
        const float highAlpha=1.0f-std::exp(-juce::MathConstants<float>::twoPi*4000.0f/static_cast<float>(sampleRate));
        const float lowGain=juce::Decibels::decibelsToGain(juce::jlimit(-12.0f,12.0f,params.bassDb.load()));
        const float midGain=juce::Decibels::decibelsToGain(juce::jlimit(-12.0f,12.0f,params.midDb.load()));
        const float highGain=juce::Decibels::decibelsToGain(juce::jlimit(-12.0f,12.0f,params.trebleDb.load()));
        for(int c=0;c<ch;++c)for(int i=0;i<n;++i){const auto state=(size_t)juce::jmin(c,1);const float x=b.getSample(c,i);toneLow[state]+=lowAlpha*(x-toneLow[state]);toneHigh[state]+=highAlpha*(x-toneHigh[state]);const float high=x-toneHigh[state],mid=x-toneLow[state]-high;b.setSample(c,i,toneLow[state]*lowGain+mid*midGain+high*highGain);}
    }
    ...
    const float pitchShift=params.pitchSemitones.load()+params.fineCents.load()/100.0f; if(std::abs(pitchShift)>.015f){pitch.setSemitones(pitchShift);pitch.process(b);}
    const float formantCutoff=juce::jlimit(500.0f,2200.0f,1050.0f+params.formant.load()*650.0f);const float formantAlpha=1.0f-std::exp(-juce::MathConstants<float>::twoPi*formantCutoff/static_cast<float>(sampleRate));for(int i=0;i<n;++i){const float amount=formantAmount.getNextValue();for(int c=0;c<ch;++c){const auto state=(size_t)juce::jmin(c,1);const float x=b.getSample(c,i);formantLow[state]+=formantAlpha*(x-formantLow[state]);const float high=x-formantLow[state];const float shaped=amount>=0?x+amount*(high*.75f-formantLow[state]*.12f):x+(-amount)*(formantLow[state]*.72f-high*.18f);b.setSample(c,i,shaped);}}
    ...
    const float flg=juce::jlimit(0.0f,1.0f,params.flanger.load());if(flg>0){for(int i=0;i<n;++i){const float lfo=.5f+.5f*std::sin(flangerPhase);flangerPhase=std::fmod(flangerPhase+juce::MathConstants<float>::twoPi*.28f/static_cast<float>(sampleRate),juce::MathConstants<float>::twoPi);const float delaySamples=static_cast<float>(sampleRate)*(.0008f+.0032f*lfo);for(int c=0;c<ch;++c){const float x=b.getSample(c,i);const float delayed=flangerDelay.popSample(c,delaySamples);flangerDelay.pushSample(c,x+delayed*.22f*flg);b.setSample(c,i,x*(1.0f-.35f*flg)+delayed*.7f*flg);}}}
    ...
    if(params.limiterEnabled.load()){limiter.setThreshold(-0.8f);limiter.setRelease(65);juce::dsp::AudioBlock<float> block(b);juce::dsp::ProcessContextReplacing<float> ctx(block);limiter.process(ctx);} for(int c=0;c<ch;++c)for(int i=0;i<n;++i){const auto state=(size_t)juce::jmin(c,1);const float x=b.getSample(c,i);const float y=x-dcX[state]+.995f*dcY[state];dcX[state]=x;dcY[state]=y;b.setSample(c,i,juce::jlimit(-1.0f,1.0f,y));} outPeak.store(b.getMagnitude(0,n),std::memory_order_relaxed);
}
}
