...
class DSPTests final:public juce::UnitTest{public:DSPTests():UnitTest("DSP"){}void runTest()override{
    ...
    beginTest("Categorias acompanham a fonte de presets");expectEquals(pm.categoryFor("Masculina grave"),juce::String("Graves"));expectEquals(pm.categoryFor(vox::PresetManager::generatedName(7)),vox::PresetManager::generatedCategory(7));expect(pm.names().contains("Voz normal limpa"));
